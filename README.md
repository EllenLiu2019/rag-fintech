...
**GPU 可观测性**：DCGM Exporter (GPU 硬件) + Prometheus (vLLM 引擎指标) + LangSmith (调用链追踪) + Grafana (统一大盘)。压测发现 9B 在 A10 上 Compute-Bound (SM Active 90%)，切换到 L20 后 SM 降至 33%，TTFT 改善 2x。

> 💡 **深度技术分享**：关于 FoC 架构的演进、工程壁垒与性能博弈，请阅读 [《架构演进：在金融 RAG 中引入「条款森林」(FoC)》](docs/adr/FoC_sharing.md)。
...
*测试环境：Qwen3.5-35B-A3B, RTX PRO 6000 96GiB, ~6K input tokens*

| 并发 | TTFT P99 (Disable) | TTFT P99 (Enable) | 提升倍数 | E2E (Enable) |
|------|---------------|--------------|---------|-------------|
| Cache 命中单请求 | 227 ms | 111 ms | **2x** | 973 ms |
| c=4 | ~249 ms | ~205 ms | **1.2x** | ~1.9s |
| c=8 | ~995 ms | ~497 ms | **2x** | ~1.9s |
| c=16 | ~2.47 s | ~747 ms | **3.5x** | ~4.97s |
| c=22 | ~4,183 ms | ~747 ms | **7x** | ~4.98s |
| c=28 | - | ~747 ms | — | ~4.97s |
| c=32 | - | ~2.4 s | — | ~4.97s |

> **工程决策**：A10 在 9B 下算力触顶，使用 35B MoE 量化模型，选择 L20 作为主力推理卡；在提升算力，使用 RTX PRO 6000 显卡后，可更换为 35B MoE 非量化模型，性能表现强劲，FoC 长上下文场景全面开启 `--enable-prefix-caching`，可用并发上限提升至 c=32+。
