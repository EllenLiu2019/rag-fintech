...
class ClaimsOrchestrator:
    ...
    async def evaluate_claim(self, doc_id: str) -> ClaimDecision:
        ...
        evidence = await self._match_clauses(request.medical_entities, request.policy_doc_id, request.claim_type)
    ...
    async def _match_clauses(
        self, entities: List[MedicalEntity], policy_doc_id: str, claim_type: str
    ) -> Dict[str, Any]:
        return await self.clause_matcher.match(entities=entities, doc_id=policy_doc_id, claim_type=claim_type)
