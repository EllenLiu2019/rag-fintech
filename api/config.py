...
class Settings(BaseSettings):
    ...
    API_DESCRIPTION: str = Field(
        default="RAG 智能洞察后端 API - 提供文件上传、解析和内容提取服务", description="API 描述"
    )
    ...
    # ========== CORS configuration ==========
    CORS_ORIGINS: List[str] = Field(default=["http://localhost:5173"], description="允许的 CORS 源地址列表")
    ...
    ALLOWED_FILE_EXTENSIONS: List[str] = Field(
        default=[".txt", ".pdf", ".doc", ".docx", ".md"], description="允许的文件扩展名"
    )

    # ========== Logging configuration ==========
    LOG_LEVEL: str = Field(default="INFO", description="日志级别")
    LOG_FORMAT: str = Field(
        default="%(asctime)s - %(name)s - %(levelname)s - [%(request_id)s] - %(message)s", description="日志格式"
    )
