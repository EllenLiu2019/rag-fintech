...
class PDFParser(BaseParser):
    ...
    def _create_llm_parser(self) -> LlamaParse:
        """Create a configured LlamaParse instance."""
        return LlamaParse(
    ...
    def _log_parse_result(self, markdown_documents: List[Document]) -> None:
        """Log a preview of the parsed result."""
    ...
    async def aparse_content(self, contents: bytes, filename: str, content_type: Optional[str]) -> ParseResult:
        """Native async version using LlamaParse.aparse() directly on the event loop."""
        extra_info = {"file_name": filename, "content_type": content_type, "size": len(contents)}
        logger.info(f"extract document info: {extra_info}")
        ...
        try:
            ...
            logger.info("LlamaParse async parse job starting...")
            result = await llm_parser.aparse(contents, extra_info=extra_info)

            markdown_documents = await result.aget_markdown_documents(split_by_page=True)
            self._log_parse_result(markdown_documents)
