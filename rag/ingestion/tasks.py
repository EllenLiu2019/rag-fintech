...
class IngestionJob(BaseModel):
    ...
    file_location: str = Field(default="", description="file_location")
...
def get_task(job_id: str) -> IngestionJob:
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"Failed to get progress for job {job_id}: {e}")
...
def update_progress(job_id: str, step: int, message: str):
    ...
    try:
        ...
        logger.debug(f"Updated progress for job {job_id}: step={step}, message={message}")
    except Exception as e:
        # Log but don't raise - progress update failure shouldn't stop ingestion
        logger.warning(f"Failed to update job progress {job_id}: {e}")
