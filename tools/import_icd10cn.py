...
def insert_icd10cn_data(
    ...
    """
    Insert ICD10CN data with graceful degradation.
    ...
    """
    snomed_path = os.path.join(os.path.dirname(__file__), "data", "ICD10CN_WITH_FSN.csv")
    ...
    with tqdm(total=total_batches, desc="Inserting SNOMED CT", unit="batch") as pbar:
        for batch_idx, chunk in enumerate(chunk_iter):
            ...
            for idx, (_, row) in enumerate(chunk.iterrows()):
                fsn = safe_str(row.get("FSN", ""))
                concept_id = safe_str(row.get("concept_id", ""))
                if fsn:
                    texts_to_embed.append(fsn)
